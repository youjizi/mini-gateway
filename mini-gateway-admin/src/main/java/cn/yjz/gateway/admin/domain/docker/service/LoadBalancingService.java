package cn.yjz.gateway.admin.domain.docker.service;
...
public class LoadBalancingService extends AbstractLoadBalancing {
    ...
    @Value("${nginx.file_path}")
    private String nginx_file_path;
    ...
    protected String createNginxConfigFile(NginxConfig nginxConfig) throws IOException {
        ...
        File file = new File(nginx_file_path+"/nginx/nginx.conf");
        if (!file.exists()) {
            // 创建目录
            File parentDir = new File(nginx_file_path+"/nginx");
